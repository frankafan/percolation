...
trials = 10000  # number of trials to repeat in order to obtain the percentage
...
def update_full(x_, y_, vertical_):  # same as percolation.py
    if x_ < 0 or x_ >= N:
        return None
    elif y_ < 0 or y_ >= N:
        return None
...
percolation_rates = []  # array to store probabilities
j = 1
for P in np.arange(0.1, 1, 0.01):  # iterate through different probabilities
    percolations = 0
    for i in range(trials):  # iterate through trials
        ...
        percolates = False  # if the system percolates, calculate likelihood.
        for x in range(N):
            if full[N - 1][x] == 1:
                percolates = True
        # print(percolates)
