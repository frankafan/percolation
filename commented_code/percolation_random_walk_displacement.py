...
def update_full(x_, y_, id_):  # same as percolation_random_walk.py
    if x_ < 0 or x_ >= N:
        return None
    if y_ < 0 or y_ >= N:
        return None
...
def next_move(x_, y_, full_):  # same as percolation_random_walk.py
...
for P in np.arange(0.4, 0.8, 0.1):  # for varying P values
    print(P)
    open = np.zeros((N, N))  # same as percolation_clusters.py
    full = np.zeros((N, N))
    for y in range(N):
        for x in range(N):
            if random.random() < P:
    ...
    id = 0  # same as percolation_clusters.py
    for y in range(N):
        for x in range(N):
    ...
    paths_x = []  # same as percolation_random_walk.py
    paths_y = []
    displacements = []  # array to store displacement at each step
    for i in range(particles):  # same as percolation_random_walk.py
        ...
        for j in range(steps):
            ...
            path_x.append(xp)
            path_y.append(yp)
            if j == 0:  # if this is the first step in the iteration
                if xp == start_x and yp == start_y:  # if the particle remains stationary after the first step, meaning the cluster size is 1
                    break  # exit the loop as the results are statistically skewing
            displacement.append(
                np.sqrt((start_x - xp) ** 2 + (start_y - yp) ** 2))  # calculate displacement using Pythagorean theorem
    ...
    for i in range(particles):
        for j in range(steps):
            if len(displacements[i]):
                displacement_total[j] += displacements[i][j]  # sum the total displacements of all the particles
